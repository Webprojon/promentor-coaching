      ...
      {history.length > 0 ? (
        <>
          ...
          <div className="mt-3 grid gap-3 max-h-[220px] overflow-y-auto hide-scrollbar">
            {history.map(({id, title, status, priority, detail}) => (
                <article key={id} className="rounded-lg border border-white/10 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Typography component="p" className="text-sm font-semibold text-white">
                        {title}
                      </Typography>
                      <Badge toneClassName={SUGGESTION_STATUS_BADGE_CLASS[status]}>
                        {status}
                      </Badge>
                    </div>
        ...
        <div className="p-4 text-center">
          ...
          <Typography component="p" className="mt-1 text-xs text-slate-400">
            Share your first idea and your history will start building right here.
          </Typography>
        </div>
      )}
    </section>
